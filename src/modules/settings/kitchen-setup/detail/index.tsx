...
import { updateBar, getDetailBar, deletBar } from "@/api/kitchen.service";
...
const DetailKitchen = () => {
  ...
  const { data, isLoading, refetch } = useQuery(["DETAIL"], () =>
    getDetailBar(id, branchId)
  );

  const barData = data?.data?.barDto;
  ...
  const {
    ...
    defaultValues: {
      name: barData?.name || "",
      isDefault: barData?.isDefault || false,
    },
  });
  ...
  useEffect(() => {
    if (barData) {
      reset({
        name: barData?.name,
        isDefault: barData?.isDefault,
      });
    }
  }, [barData, reset]);

  const { mutate: createCustomerMutation, isLoading: isUpdating } = useMutation(
    (data: any) => updateBar(id, data),
    {
      onSuccess: () => {
        message.success("Cập nhật bếp thành công!");
